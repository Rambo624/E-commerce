        ...
        {addresses.length > 0 ? (
          addresses.map((address) => (
            <div key={address._id} className='p-4 border rounded shadow'>
              <h2 className='text-xl'>{address.House}</h2>
              <p>{`${address.city}, ${address.state} ${address.pin}`}</p>
              <button onClick={() => handleEdit(address._id)} className='p-1 bg-blue-600 text-white rounded-lg mr-5 mt-3'>EDIT</button>
              <button onClick={() => handleDelete(address._id)} className='p-1 mt-3 bg-red-600 text-white rounded-lg'>Remove</button>
            </div>
          ))
        ) : (
      ...
      <form onSubmit={handleSubmit} className='mt-6'>
        ...
        <div className='grid grid-cols-1 gap-4'>
          ...
          <input 
            ...
            required
          />
          <button type='submit' className='px-4 py-2 w-60 bg-blue-500 text-white rounded hover:bg-blue-600'>
            Add Address
          </button>
        </div>
      </form>
